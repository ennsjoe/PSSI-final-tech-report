...
if (!is.na(project$references) && nchar(project$references) > 0) {
  ...
  # Split references by line breaks (assuming each reference is on its own line or paragraph)
  refs <- strsplit(project$references, "\n\n")[[1]]
  refs <- trimws(refs)
  refs <- refs[refs != ""]
  
  # Wrap each reference in a div with custom style
  for (ref in refs) {
