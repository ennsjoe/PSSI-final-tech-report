...
render_word_table <- function(
  ...
  # Caption — styled paragraph before the table
  cat("::: {custom-style=\"Caption\"}\n")
  if (!is.null(table_number)) {
    cat(paste0("Table ", table_number, ". ", caption, "\n"))
  } else {
...
if (!is.null(safe_text(project$sub_region)))
  metadata_items <- c(metadata_items,
                      paste0("**Region:** ", safe_text(project$sub_region)))
...
if (!is.null(safe_text(project$focus_population)))
  metadata_items <- c(metadata_items,
                      paste0("**Populations:** ", safe_text(project$focus_population)))
...
# ── Tables and figures (after Next Steps) ─────────────────────────────────────
