...
render_word_table <- function(
  ...
  if (is.null(data) || nrow(data) == 0 || ncol(data) == 0) {
    if (!is.null(table_number)) {
      cat(paste0("Table ", table_number, ". ", caption, "\n\n"))
    } else {
      cat(caption, "\n\n")
    }
  ...
  # Caption — output with custom "Caption" style (matches figure caption styling)
  if (!is.null(table_number)) {
...
if (has_files || !is.null(tables_figures_text)) {
  ...
  if (has_files) {
    ...
    for (file_path in c(table_files, img_files)) {
      ...
      # Image files named Table/Tbl should be captioned as tables, not figures
      is_table_image <- file_ext %in% c("png", "jpg", "jpeg", "gif", "svg", "tif", "tiff") &&
                        !is.null(table_number)
      ...
      tryCatch({
        if (file_ext %in% c("xlsx", "xls")) {
          ...
          cat("::: {custom-style=\"Caption\"}\n")
          if (is_table_image) {
            # Image named as a table — caption as Table X.
            cat(paste0("Table ", table_number, ". ", caption, "\n"))
          } else {
            # Regular figure
            ...
            cat(paste0("Figure ", figure_number, ". ", caption, "\n"))
          }
          cat(":::\n\n")
        }
