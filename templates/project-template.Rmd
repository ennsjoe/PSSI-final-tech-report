...
# Table rendering function with proper Word table formatting
render_word_table <- function(data, caption, table_number = NULL) {
  ...
  # Clean column names for display (remove underscores, etc.)
  display_names <- names(clean_data)
  ...
  max_rows <- 40
  if (nrow(clean_data) > max_rows) {
    clean_data <- clean_data[1:max_rows, ]
    truncated <- TRUE
  } else {
    truncated <- FALSE
  }
  
  # Display caption with table number (NOT BOLD)
  if (!is.null(table_number)) {
    cat("Table ", table_number, ". ", caption, "\n\n")
  } else {
    cat(caption, "\n\n")
  ...
  names(clean_data) <- display_names
  
  # Try pandoc format first (best for Word tables with borders)
  tryCatch({
    ...
    message("Pandoc table failed, trying pipe format: ", e$message)
    
    # Fallback: try pipe format
    tryCatch({
      ...
      message("All table formats failed, using text fallback: ", e2$message)
      
      # Final fallback: simple text table
      cat("```\n")
      
      # Calculate column widths
      col_widths <- integer(ncol(clean_data))
      for (i in 1:ncol(clean_data)) {
        col_widths[i] <- max(
          nchar(display_names[i]),
          if (nrow(clean_data) > 0) max(nchar(clean_data[[i]]), na.rm = TRUE) else 0,
          na.rm = TRUE
        )
        col_widths[i] <- min(col_widths[i], 25)  # Cap width
      }
      
      # Header
      header_parts <- character(ncol(clean_data))
      for (i in 1:ncol(clean_data)) {
        header_parts[i] <- sprintf(paste0("%-", col_widths[i], "s"), 
                                  substr(display_names[i], 1, col_widths[i]))
      }
      cat(paste(header_parts, collapse = " | "), "\n")
      
      # Separator line
      sep_parts <- character(ncol(clean_data))
      for (i in 1:ncol(clean_data)) {
        sep_parts[i] <- paste(rep("-", col_widths[i]), collapse = "")
      }
      cat(paste(sep_parts, collapse = "-|-"), "\n")
      
      # Data rows
      display_rows <- min(nrow(clean_data), 20)
      for (i in 1:display_rows) {
        row_parts <- character(ncol(clean_data))
        for (j in 1:ncol(clean_data)) {
          cell_value <- substr(clean_data[i, j], 1, col_widths[j])
          row_parts[j] <- sprintf(paste0("%-", col_widths[j], "s"), cell_value)
        }
        cat(paste(row_parts, collapse = " | "), "\n")
      }
      
      if (nrow(clean_data) > display_rows) {
        cat("... (", nrow(clean_data) - display_rows, " more rows)\n")
      }
      cat("```\n")
