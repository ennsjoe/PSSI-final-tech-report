...
# Placeholder text to treat as NA
PLACEHOLDER_TEXT <- "Click or tap here to enter text."

#' Get all text from an XML node
...
clean_value <- function(value) {
  ...
  cleaned <- str_squish(value)
  
  # Return NA if empty or contains placeholder text
  if (nchar(cleaned) == 0 || 
      str_detect(cleaned, fixed(PLACEHOLDER_TEXT)) ||
      cleaned == PLACEHOLDER_TEXT) {
    return(NA_character_)
  }
  
  # Remove any remaining placeholder text fragments
  cleaned <- str_remove_all(cleaned, fixed(PLACEHOLDER_TEXT))
